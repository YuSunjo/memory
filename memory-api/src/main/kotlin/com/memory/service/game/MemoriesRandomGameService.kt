...
class MemoriesRandomGameService(
    ...
    private val log = LoggerFactory.getLogger(MemoriesRandomGameService::class.java)
    ...
    override fun createGameSession(
        ...
        val availableMemories: List<Memory?> =
            memoryRepository.findMemoriesWithImagesByMemoryType(MemoryType.PUBLIC)
        check(availableMemories.size >= MIN_MEMORIES_FOR_GAME) {
            String.format(
                "랜덤 게임을 시작하기 위해서는 최소 %d개의 공개 추억이 필요합니다.",
                MIN_MEMORIES_FOR_GAME
            )
        }


        // GameSession 생성 (targetMember는 null)
        val gameSession = gameSessionInit(member, GameMode.MEMORIES_RANDOM)
