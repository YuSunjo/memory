...
enum class NotificationType(
    ...
    fun getFirebaseType(): String = this.name.lowercase()

    fun isImmediateDelivery(): Boolean = when (this) {
        MEMORY_CREATED, MEMORY_SHARED, RELATIONSHIP_REQUEST,
        GAME_INVITATION, CALENDAR_REMINDER -> true
        else -> false
    }

    fun isBatchable(): Boolean = when (this) {
        MEMORY_LIKED, MEMORY_COMMENTED, TODO_DEADLINE,
        SYSTEM_NOTICE -> true
        else -> false
    }
}
